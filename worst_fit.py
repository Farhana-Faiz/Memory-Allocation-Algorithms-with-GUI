        ...
        for j in range(len(memory_blocks)):
            if memory_blocks[j] >= process_sizes[i]:  # Check if the block is large enough
                if worst_index == -1 or memory_blocks[j] > memory_blocks[worst_index]:
                    worst_index = j  # Find the block with the maximum unused space

        if worst_index != -1:  # If a suitable block was found
            allocation[i] = worst_index  # Assign the block to the process
            memory_blocks[worst_index] -= process_sizes[i]  # Reduce available memory in the block
...
# Title label
title_label = tk.Label(root, text="Worst-Fit Memory Allocation Algorithm", font=("Arial", 16, "bold"), pady=10)
title_label.pack()
